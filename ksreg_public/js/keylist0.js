        $(document).ready(function(){
            ...
            $(".saveintro").bind("click",function(){
				...
				var pdata="id="+kid+"&ext1="+$("#ext1"+kid).val()+"&dayactivetimes="+$("#dayactivetimes"+kid).val()+"&int1="+$("#int1"+kid).val()+"&v7ext1="+$("#v7ext1"+kid).val()+"&linknum="+$("#linknum"+kid).val()+"&ktname="+gkeyt_ext+"&intro="+$("#intro"+kid).val();
				$.ajax({type: "POST",url: "?action=saveintro&rand="+sTime2(),data:pdata,success: function(msg){
				if(msg=="ok"){
					$("#tdext1"+kid).html($("#ext1"+kid).val());
					$("#tdint1"+kid).html($("#int1"+kid).val()+'|'+$("#v7ext1"+kid).val());
					$("#tddayactivetimes"+kid).html($("#dayactivetimes"+kid).val());
					$("#tdlinknum"+kid).html($("#linknum"+kid).val());
					objtext0.fadeIn("fast");
					objtext0.text("保存成功！");
					objtext0.fadeOut(2000);}else{alert(msg)}
				},error:function(ajaxobj){	alert("出错："+ajaxobj.status);}}); 
			});
					
            $(".jsbtn").bind("click",function(){
				var keyid=$(this).attr("kid");
				var keyaddtime=$("#js"+keyid).attr("value");
				$.getScript("?action=keyaddtime&ktname="+gkeyt_ext+"&keyid=" + keyid + "&keyaddtime=" + keyaddtime+"&rand="+sTime2());
			});
            ...
            $("a[ac='jb']").bind("click",function(){
				...
				$.ajax({type: "GET",url: ohref+"&ajax=1&rand="+sTime2(),
					success: function(msg){
						if(msg=="ok"){
							$.msgbox({width:300,height:120,content:{type:'alert', content:'解绑成功'},autoClose:2});
						}else{
							$.msgbox({width:300,height:120,title:'出错',content:{type:'alert', content:msg}});
						}
            ...
            $("a[ac='del']").bind("click",function(){
				...
				$.msgbox({
					...
					width:250,
					animation:0,
					content:{type:'confirm', content: '确认删除该注册卡?'},
					onClose:function(v){
						if(v){
								$.ajax({type: "GET",url: ohref+"&ajax=1&rand="+sTime2(),
									success: function(msg){
			...
			$("input[gl]").keyup(function(){this.value=this.value.replace(/[^A-Za-z0-9_\-]/g,'');});
